...
| `fulltext` | 仅实体名全文精准检索 | 3 / 19 | 15.8% | — |
| `hybrid` | 向量 + 全文 RRF 融合 | 9 / 19 | 47.4% | 0.0% |
| **`hybrid_graph`** | **向量 + 全文 + KG²RAG 图扩展 (最终方案)** | **14 / 19** | **73.7%** | **+26.3pp** (命中数 +56%) |
...
| q11 | suggest | 拥有蓄水特性的宝可梦有哪些？ | ✗ | ✓ | ✓ | ✓ | 别名归一将“蓄水”对齐到“储水”后，全文与混合路径可直达 |
...
### 核心实验发现
1. **KG²RAG 产生质的飞跃**：在纯克制和弱点查找题（`q04`, `q05`, `q06`, `q12`）中，纯向量与纯全文检索在 41507 条文本块语料库中完全无法召回对应属性的克制候选或精准受击说明；**KG²RAG 沿图谱结构扩展后，4 道题全部从 0 翻盘至命中**；
...
`src/templates/index.html` 顶部工具栏提供“检索路径”下拉框，可在 `vector` / `fulltext` / `hybrid` / `hybrid_graph` 四条路径间实时切换：

- 回答区徽标（“检索：仅向量 / 仅全文 / 混合检索 / 混合+图扩展”）与证据区标题同步显示当前路径，现场演示可直接对照四路召回差异；
- 对照模式下 naive 与 GraphRAG 两条链路使用同一检索路径，用于隔离“图事实注入”这一单一变量；
- 后端 `/api/ask`、`/api/compare` 均接受 `retrieval_mode` 参数，非法取值统一回落到 `hybrid_graph`，避免演示时误操作导致接口报错。
