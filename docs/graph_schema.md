...
| `Chunk` | `chunk_id` | `kind`、`entity_type`、`entity_id`、`name_zh`、`text`、`embedding`、`embed_model` | `build_engine.py` 生成文本块，`embed_vectors.py` 写向量 |
...
`HITS_TYPE` 方向统一为“攻击属性 -> 防御属性”，`multiplier` 可取 `0/0.25/0.5/1/2/4`；双属性防守方由查询端对两个防御属性倍率相乘。
...
- `Chunk` 目前只导入 `entity_chunks`；`relation_chunks` 作为可选语料，不重复向量化，避免索引膨胀。
- 叙事关系 `RIVAL_OF/PREDATES_ON/...` 不在当前确定性建库中；`multi_qa.py/rag.py` 保留查询，只有库中存在这些边时才生效。

## 5. 规模基线

以下为 2026-09-09 组内 Aura 库 `check_graph.py` 实测；原始 JSONL 含少量同 id 重复（上游 list 重复），导入按 id 去重。
...
- 文本块：entity_chunks 30728；relation_chunks 96238（当前向量语料只导入 entity_chunks）
...
- 唯一约束：`Pokemon.pokedex_id`、`Chunk.chunk_id`。

`load_engine_out.py` 的节点导入以各标签 `id` 为合并键；Form 等节点的 `id` 唯一性建议后续补唯一约束，当前由 `MERGE` 保证幂等。
