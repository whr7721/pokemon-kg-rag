...
> **关于本仓库**：天津大学《知识工程综合实践》课程团队项目的归档镜像。本人（[@whr7721](https://github.com/whr7721)，成员四）负责**应用与前端**：Flask 接口与两级路由入口、问答前端（示例问题 / loading / 错误提示 / 子图图例 / 模式徽章），并对全链路做了完整复现与文档化；各成员的开发轨迹见 `feat/*` 分支与 `docs/` 各阶段总结。
