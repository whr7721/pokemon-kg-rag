...
class RagRouter:
    ...
    def answer(self, question, top_k=8, use_graph=True):
        structured = self.mqa.answer(question)
        if structured is not None:
            evidence = self.rag.retrieve(question, top_k=top_k)
            return {
                "question": question,
        ...
        result = self.rag.ask(question, top_k=top_k, use_graph=use_graph)
        return {
            "question": question,
